...
class BingX:
        ...
        def on_message(ws, message):
            try:
                ...
                if "data" in data:
                    ...
                    print(f"\nExchange: {self.exchange}, Symbol: {symbol}, Temporalidad: {interval}\nInformación de vela actual:")
                    print(f"{df_data}")
    ...
    def check_strategy(self, last_price: float):
        ...
        decimales = self.cant_deci_precio()
        #pip_precio = self.pip_precio()
        estrategia = self.estrategia(df_dynamic, last_price, decimales)

        if positionside == "LONG":

            resultado = estrategia.evaluar_entrada_long()
            if resultado.get("estrategia_valida", False):
                # Llamada a la función para colocar la orden de mercado o limit
                self.set_limit_market_order(self.symbol, positionside, self.modo_gestion, resultado)
            ...
            resultado = estrategia.evaluar_entrada_short()
            if resultado.get("estrategia_valida", False):
                # Llamada a la función para colocar la orden de mercado o limit
                self.set_limit_market_order(self.symbol, positionside, self.modo_gestion, resultado)
...
if __name__ == "__main__":
    ...
    #MonitorMemoria = mgo.Monitor_Memoria() # Clase para monitorear el uso de memoria
    # Varibles de la clase BingX()
    #estrategia = SMA_MACD_BB
    estrategia = SMA_BB
    ...
    try:
        ...
        bingx._iniciar_monitor_memoria()
